// Frozen accepted-zoo baseline. This is a self-contained reference: it exercises
// only the preserved monolith renderShowcase against an inline manifest fixture, so
// it can run standalone from this directory without the build harness. It is NOT
// part of the live suite (vitest excludes design-system/reference/**); it documents
// the accepted behaviour the modular rebuild must reproduce.
...
describe('showcase (the zoo)', () => {
  ...
  it('inlines all three fonts and stays self-contained', () => {
    const h = html({ fontCss: "@font-face{font-family:'Archivo';src:url(data:font/woff2;base64,AAAA)}@font-face{font-family:'JetBrains Mono';src:url(data:font/woff2;base64,AAAA)}@font-face{font-family:'Caveat';src:url(data:font/woff2;base64,AAAA)}" });
    expect((h.match(/@font-face/g) || []).length).toBe(3); // Archivo, JetBrains Mono, Caveat
    expect(h).toContain("font-family: 'Caveat'");
  });
});
