// Illustrative demo skins. Each overrides colour roles only — never type or
// spacing. `color-ink` (the skin's ink, same value as its default text) is
// included so the ink-anchored chains the built CSS keeps live — the button
// halftone (var(--color-ink)), inverted ink surfaces (color.surface.ink), the
// default text — all reskin from the one override. `color-focus-ring` mirrors
// the skin accent for the same reason: the focus-ring role bottoms out in the
// red primitive, so the demo re-supplies it (per its derivation, focus-ring is
// the accent — see language/colour.derivations.json).
export const SKINS = [
