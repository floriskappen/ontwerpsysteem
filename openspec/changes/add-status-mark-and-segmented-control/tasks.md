...
- [ ] 1.1 Add `state.mark.static` to `design-system/recipes/state.recipes.json` with the
...
- [ ] 1.2 Add `component.tabs.segmented` to `design-system/recipes/component.recipes.json`
...
- [ ] 1.3 Recompile `design-system/recipes/index.json` via the build so both recipes appear
      in the compiled index. Named check: recipe-index compilation includes both IDs.

## 2. Language prose

- [ ] 2.1 Add the status mark to `design-system/language/states.md`, stating explicitly
...
- [ ] 2.2 Add the segmented control to `design-system/language/components.md` as a reusable
...
- [ ] 3.1 Add the status mark and segmented control to
...
- [ ] 3.2 Add class-rooted styles to `design-system/source/zoo/styles/components.css`: a
      ...
      switch is immediate*).
- [ ] 3.3 Confirm no green-check / checkbox glyph is used in the status-mark markup or CSS
...
- [ ] 4.1 `npm run validate` passes: cross-layer reference validation resolves both
...
- [ ] 4.2 `npm run build` succeeds and rebuilds the zoo; confirm the components section
      ...
      role, not the accent*).
- [ ] 4.3 `openspec validate add-status-mark-and-segmented-control --strict` passes.
